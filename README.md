...
1. **虚拟终端** - 在应用内嵌入终端，直接查看 Claude Code 执行输出
2. **双向交互** - 支持向 Claude 发送语音/文字消息，实现远程对话
3. **macOS 代码签名** - 完成开发者证书签名，消除"文件已损坏"警告
4. **iOS TestFlight** - Mobile App 上架 TestFlight，支持 iOS 用户测试

### 中期规划

5. **移动端 Push 通知** - iOS/Android 推送权限请求，无需打开 App 即可响应
6. **Device 消息聚合** - 单页面聚合所有 Claude 实例的消息和状态总览
7. **快捷指令库** - 预设常用回复模板，一键发送给 Claude
...
13. **团队协作** - 多用户共享监控，支持权限委托
14. **会话分享** - 将精彩对话分享给团队成员
15. **安全策略** - 配置白名单/黑名单，自动审批信任工具
16. **审批审计** - 记录所有审批决策，支持回溯分析
17. **AI 审批建议** - 分析操作风险，智能推荐审批决策
18. **VS Code 插件** - IDE 内嵌监控，无需切换窗口
19. **异常检测** - 识别可疑操作模式，主动告警
20. **OpenTelemetry 支持** - 导出追踪数据，对接 Grafana/Prometheus 等监控平台
